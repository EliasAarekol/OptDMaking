...
class Q_network(Critic): # Why isnt this camelcase
    def __init__(self,dims,lr,df,eps,tau,device):
        ...
        self.device = device
        self.lr = lr
        self.target = DQN(self.dims,device=self.device)
        self.policy = DQN(self.dims,device=self.device)
    ...
    def train(self,rewards,actions,states,nxt_states):
        ...
        old_norm = float('inf')
        while  abs(old_norm-torch.linalg.norm(torch.cat([param.flatten() for param in self.policy.parameters()])) ) > self.eps:
            old_norm = torch.linalg.norm(torch.cat([param.flatten() for param in self.policy.parameters()]))
            ...
            state_action_values = self.policy(state_batch).gather(1, action_batch.unsqueeze(1))
    ...
    def evaluate(self, actions, states):
        action_batch = torch.tensor(actions,device=self.device)
        state_batch = torch.tensor(states,device=self.device).squeeze().float()
        return self.policy(state_batch).gather(1, action_batch.unsqueeze(1)).cpu().detach().numpy()
