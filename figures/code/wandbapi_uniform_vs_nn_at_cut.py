...
for seed, group in groupby(runs_sorted, lambda x: x.name.strip()[-1]):
    ...
    for df_run_to_plot in run_data_list:
        ...
        col_data = col_data.rolling(window=window_size, min_periods=1).mean()
        ...
        smoothed_normalized_data = normalized_data
        label = "uniform" if "naive" in df_run_to_plot['run_name'].iloc[0] else "nn"
        color = color_map[label]
        plt.plot(df_run_to_plot["_step"].iloc[:], smoothed_normalized_data.iloc[:], label=label,color = color)
        # loc_fig = plt.gcf()
...
fig.subplots_adjust(
    ...
    hspace=1.2,  # vertical spacing between subplots
    wspace=0.4   # horizontal spacing between subplots
)
# fig.patch.set_linewidth(1)
# fig.patch.set_edgecolor('grey')

filename = os.path.join(output_dir, 'uniform_vs_nn_at_pruned.pgf')
