...
runs_api = api.runs(
    ...
    filters={"$or": [{"group": "unknown_c_unknown_value_at_fathomed_naive"},
                     {"group": "unknown_c_unknown_value_at_sample_naive"}]},
)
...
plt.tight_layout()
plt.suptitle("Gradient sampling comparison, uniform")
plt.show()

filename = os.path.join(output_dir, 'per_subplot_normalized_comparison.png')
